...
const FILTER_GROUPS = [
  ...
  {
    label: 'LEGO',
    filters: [{ label: 'LEGO Kits', value: 'legokit' }],
  },
]
...
const Shop = () => {
  ...
  const filtered = useMemo(() => {
    let r = [...validEntries]
    if      (activeFilter === 'new')        r = r.filter(e => e.banner?.value?.toLowerCase().includes('new'))
    else if (activeFilter === 'leaving')    r = r.filter(e => { if (!e.outDate) return false; return new Date(e.outDate).getTime() - Date.now() < 86400000 })
    else if (activeFilter === 'bundle')     r = r.filter(e => !!e.bundle?.name)
    else if (activeFilter === 'outfit')     r = r.filter(e => e.brItems?.[0]?.type?.value === 'outfit')
    else if (activeFilter === 'pickaxe')    r = r.filter(e => e.brItems?.[0]?.type?.value === 'pickaxe')
    else if (activeFilter === 'glider')     r = r.filter(e => e.brItems?.[0]?.type?.value === 'glider')
    else if (activeFilter === 'backpack')   r = r.filter(e => e.brItems?.[0]?.type?.value === 'backpack')
    else if (activeFilter === 'wrap')       r = r.filter(e => e.brItems?.[0]?.type?.value === 'wrap')
    else if (activeFilter === 'shoe')       r = r.filter(e => e.brItems?.[0]?.type?.value === 'shoe')
    else if (activeFilter === 'sidekick')   r = r.filter(e => e.brItems?.[0]?.type?.value === 'sidekick')
    else if (activeFilter === 'emote')      r = r.filter(e => ['emote','spray','toy'].includes(e.brItems?.[0]?.type?.value ?? ''))
    else if (activeFilter === 'track')      r = r.filter(e => !!e.tracks?.length)
    else if (activeFilter === 'instrument') r = r.filter(e => !!e.instruments?.length)
    else if (activeFilter === 'legokit')    r = r.filter(e => !!e.legoKits?.length)
    else if (activeFilter === 'car')        r = r.filter(e => !!e.cars?.length)
  ...
  return (
    <div className="min-h-screen text-white flex flex-col"
        ...
        <div className="max-w-screen-2xl mx-auto px-4 lg:px-6">
          ...
          <div className="flex items-center justify-between py-3 border-b border-white/5">
            <div className="flex items-center gap-2">
              <img src="https://fortnite-api.com/images/vbuck.png" alt="" className="w-6 h-6" />
              <span className="font-black text-base lg:text-lg tracking-widest uppercase">Item Shop</span>
            </div>
            <p className="text-white font-bold text-xs lg:text-sm hidden sm:block">{today}</p>
          ...
          <div className="py-2.5 flex items-center gap-2 flex-wrap">
            ...
            {newCount > 0 && (
              <button
                ...
                className={`font-bold text-xs px-2 py-1 rounded-full border transition-all ${
                  activeFilter === 'new'
            ...
            {/* FILTER — fixed position dropdown on mobile */}
            <div className="relative" ref={filterRef}>
              ...
              >
                ...
                <svg className={`w-3 h-3 transition-transform ${filterOpen ? 'rotate-180' : ''}`}
                      ...
                      <div className="grid grid-cols-3 gap-1.5">
                        {group.filters.map(f => (
                          <button
                            ...
                            className={`px-2 py-2 rounded-xl text-xs font-bold text-left transition-all border ${
            ...
            <select
              ...
              className="bg-white/5 border border-white/10 text-white text-xs font-bold rounded-xl px-3 py-2 outline-none cursor-pointer hover:bg-white/10"
      ...
      <div className="flex flex-1 relative">
        ...
        {/* EXPANDABLE DOT SIDEBAR — desktop only */}
                ...
                >
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor: isActive ? accent.dot : sidebarHover ? accent.dot : 'rgba(255,255,255,0.25)',
                      boxShadow: isActive ? `0 0 8px ${accent.dot}` : 'none',
                    }}
                  />
                  {sidebarHover && (
                    <span className={`text-xs font-bold truncate whitespace-nowrap ${isActive ? accent.text : 'text-gray-400'}`}>
                      {group.name}
                    </span>
                  )}
                </button>
        ...
        {/* MAIN */}
        <main className="flex-1 max-w-screen-2xl mx-auto w-full px-3 lg:px-8 py-6 lg:pl-10">
          {loading && (
          ...
          {!loading && !error && grouped.map((group, groupIndex) => {
            ...
            return (
              ...
              >
                <div className={`flex items-center gap-3 mb-4 pb-3 border-b ${accent.border} border-opacity-20`}>
                  <div className="w-1 h-7 rounded-full shrink-0" style={{ backgroundColor: accent.dot }} />
                  <h2 className={`text-lg lg:text-2xl font-black uppercase tracking-wider ${accent.text}`}>
                    {group.name}
                  </h2>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-2 lg:gap-3">
      ...
      {!loading && !error && (
        <div className="border-t border-white/5 py-4 text-center shrink-0">
